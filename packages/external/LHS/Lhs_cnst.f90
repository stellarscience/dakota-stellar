...
C***********************************************************************
C     Last change:  SLD  11 Jul 101   11:11 am
!LHS_EXPORT_DEC ATTRIBUTES DLLEXPORT::LHS_CONST
      SUBROUTINE LHS_CONST(NAMVAR,PTVAL,IError,IPVNO)
c     LHS_CONST inputs data for a user-defined "constant" distribution
c     LHS_CONST calls routines: LJUST
c
c   Descriptions of call list parameters:
c   Inputs:
c     NAMVAR = name of the variable
c     PTVAL = real point value "constant" associated with NAMVAR
c   Outputs:
...
c
            Value = PTVAL
c take "CONSTANT" loop from RDPAR2
c        -- Check for the "Constant" distribution type
c
CCCC         If ( LCard(1:9) == 'CONSTANT ' ) Then
            IVarNm(IList) = -9999999
cc            Read (LCard(10:),*,Err=9000) Value                        sld01
CCCC            Read (LCard(10:),*,Err=9000,END=9000) Value                 sld01
CCCC            If ( IPtVal /= 0 ) PValue(IList) = Value
c
c LHS_CONST sets th point value to be the same as the constant value
c regardless of what the LHSPVAL setting is !!!!!
c Note: that this is different from the Standard LHS which will print
c       a value different than the constant value for the point value
c       if the user inputs something different from the constant
c
            PValue(IList) = Value
c
CCCC            Cycle
CCCC         End If
c
c     Set the point value index number for return to LHS_CONST
      IPVNO = IList
      RETURN
c
