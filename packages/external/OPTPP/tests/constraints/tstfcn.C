...

{
  ...
  for (int i=1; i<=ndim/4; i++) {
    ...
    f2 = sqrt(5.0) * ( x(4*i-1-1) - x(4*i-1));
    f3 = square(x(4*i-2-1) - 2*x(4*i-1-1));
    f4 = sqrt(10.0) * (x(4*i-3-1) - x(4*i-1));
...
{ 
  ...
  for (int i=1; i<=ndim; ++i) {
    ...
    for (int j=1; j<=ndim; ++j) {
      sum += cos(x(j-1)) + double(i)*(1.0-cos(x(i-1)) - sin(x(i-1)));
    }
    fi = ndim - sum;
    fx += fi*fi;
  }
...
void trig_as_a_constraint(int ndim, const SerialDenseVector<int,double>& x, 
		          SerialDenseVector<int,double>& fx, int& result)
...
void init_penalty1 (int ndim, SerialDenseVector<int,double>& x)
{
  ...
  for (int i = 1; i <= ndim; i++) x(i-1) = double(i);
...
//        f             function value at x 
  ...
  for (i=1; i <= ndim; i++) sum += square(x(i-1));
  fx = (sum - .25)*(sum - .25);
...
void init_penalty2 (int ndim, SerialDenseVector<int,double>& x)
{
  ...
  for (int i = 1; i <= ndim; i++) x(i-1) = 0.5;
...
//        f             function value at x 
  ...
  f1  = x(1-1) - 0.2;
  fx  = square(f1);
  for (i=1; i <= ndim; i++) sum += double(ndim - i + 1)*square(x(i-1));
  sum = sum - 1.0;
  fx += square(sum);
  ...
  for (i = 2; i <= ndim; i++) {
    ...
    fi = exp(x(i-1)/10.0) + exp(x(i-1-1)/10.0) - yi;
    fx += k1*square(fi);
  }
  for (i = ndim+1; i < 2*ndim; i++) {
    fi = exp(x(i-ndim+1-1)/10.0) - exp(double(-1)/10.0);
    fx += k1*square(fi);
  }
  result = NLPFunction;
}
void init_vardim (int ndim, SerialDenseVector<int,double>& x)
{
  ...
  for (int i = 1; i <= ndim; i++) x(i-1) = 1.0 - (double(i)/double(ndim));
...
//        f             function value at x 
  ...
  // Take care of the two end cases first
  //
  for (i=1; i <= ndim; i++) sum += double(i)*(x(i-1) - 1.0);
...
void init_broyden_tridiag (int ndim, SerialDenseVector<int,double>& x)
{
  ...
  for (int i = 1; i <= ndim; i++) x(i-1)   = -1.0;
...
//        f             function value at x 
  ...
  fn = (3.0 - 2.0*x(ndim-1))*x(ndim-1) - x(ndim-1-1) - 2.0*xn_plus_1 + 1.0;
  ...
  //
  for (i = 2; i < ndim; i++) {
    fi  = (3.0 - 2.0*x(i-1))*x(i-1) - x(i-1-1) - 2.0*x(i+1-1) + 1.0;
    fx += square(fi);
  }
  result = NLPFunction;
}
...
//        f             function value at x 
  ...
  fn = fabs((3.0 - 2.0*x(ndim-1))*x(ndim-1) - x(ndim-1-1) - 2.0*xn_plus_1 + 1.0);
  ...
  //
  for (i = 2; i < ndim; i++) {
    fi  = fabs((3.0 - 2.0*x(i-1))*x(i-1) - x(i-1-1) - 2.0*x(i+1-1) + 1.0);
    fx += pow(fi,p);
  }
  result = NLPFunction;
}
...
//        f             function value at x 
  ...
  for (i = 1; i < ndim; i++) {
    ...
    fi  = fabs((2.0 + 5.0*square(x(i-1)))*x(i-1) + 1.0 - sum);
    fx += pow(fi,p);
  }
  result = NLPFunction;
}
...
//        f             function value at x 
  ...
  fn = fabs((3.0 - 2.0*x(ndim-1))*x(ndim-1) - x(ndim-1-1) - xn_plus_1 + 1.0);
  ...
  //
  for (i = 2; i < ndim; i++) {
    fi  = fabs((3.0 - 2.0*x(i-1))*x(i-1) - x(i-1-1) - x(i+1-1) + 1.0);
    fx += pow(fi,p);
  }
  //
...
//        f             function value at x 
  ...
  for (i = 1; i <= ndim; i++) {
	...
	t        = beta(i-1)*x(i-1) + beta(j-1)*x(j-1) + c_ij;
	fx      += alpha_ij*sin(t);
      }
    }
  }
...
void init_chebyquad (int ndim, SerialDenseVector<int,double>& x)
{
  ...
  for (int i = 1; i <= ndim; i++) x(i-1)   = double(i)/double(ndim+1);
