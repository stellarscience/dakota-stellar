...
* Absolute path for NOWPAC_INSTALL_PREFIX expected
...
```
* Use approximate Gaussian process (Subset of Regressors, Deterministic Training Conditional, Fully Independent Training Conditional)
```
std::string use_approx_gaussian_process = "GP";
```
* Apply hard box constraints, meaning no evaluations outside the box constraints (WARNING: might influence quality of surrogate)
```
bool use_hard_box_constraints = false;
```
* Evaluate newly added evaluation points in parallel/asynchronously (BlackBoxBaseClass methods evaluate_nowait(...) and synchronize() have to be implemented by user)
```
...
// Start optimization
(int) EXIT_FLAG = OPT.optimize( (std::vector<double>&) <x>, (std::vector<double>&) <opt_value>);
```
