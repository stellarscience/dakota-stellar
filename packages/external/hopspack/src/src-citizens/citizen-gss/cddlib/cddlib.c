/*
  $Id: cddlib.c 149 2009-11-12 02:40:41Z tplante $
  $URL: https://software.sandia.gov/svn/hopspack/trunk/src/src-citizens/citizen-gss/cddlib/cddlib.c $
*/
...
dd_boolean dd_DDFile2File(char *ifile, char *ofile, dd_ErrorType *err)
{
  ...
  else if ( fopen_s(&reading,ifile, "r") == 0) {
 #else
  ...
  if (found){
    if (strcmp(ofile,"**stdout") == 0 )
      writing = stdout;
   #if defined(HAVE_MSVC_SECURE_STRING_FNS)
    else if ( fopen_s(&writing,ofile, "w") == 0){
   #else
