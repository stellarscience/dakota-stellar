/*
  $Id: cddio.c 149 2009-11-12 02:40:41Z tplante $
  $URL: https://software.sandia.gov/svn/hopspack/trunk/src/src-citizens/citizen-gss/cddlib/cddio.c $
*/
  ...
  while (!opened && !quit) {
    ...
    for (i=0; i<dd_filenamelen && !stop; i++){
      ...
      switch (ch) {
        ...
        #if defined(HAVE_MSVC_SECURE_STRING_FNS)
          strncpy_s(tempname,dd_filenamelen,inputfile,i);
          strcpy_s(inputfile,dd_filenamelen,tempname);
        #else
...
void dd_SetWriteFileName(dd_DataFileType inputfile, dd_DataFileType outfile, char cflag, dd_RepresentationType rep)
{
  ...
  dotpos=-1;
  for (i=0; i< (int) strlen(inputfile); i++){
    if (inputfile[i]=='.') dotpos=i;
  }
 #if defined(HAVE_MSVC_SECURE_STRING_FNS)
  if (dotpos>1) strncpy_s(ifilehead, dd_filenamelen, inputfile, dotpos);
  else strcpy_s(ifilehead,dd_filenamelen,inputfile);
  if (strlen(inputfile)<=0) strcpy_s(ifilehead,dd_filenamelen,"tempcdd");
  strcpy_s(outfile,dd_filenamelen,ifilehead); 
  strcat_s(outfile,dd_filenamelen,extension); 
  if (strcmp(inputfile, outfile)==0) {
    strcpy_s(outfile,dd_filenamelen,inputfile); 
    strcat_s(outfile,dd_filenamelen,extension); 
  }
 #else
...
void dd_SetLinearity(dd_MatrixPtr M, char *line)
{
 ...
 #if defined(HAVE_MSVC_SECURE_STRING_FNS)
  char * next_token_context;
  next=strtok_s(line,ct,&next_token_context);
  eqsize=atol(next); 
  while (i < eqsize && (next=strtok_s(NULL,ct,&next_token_context))!=NULL) {
     var=atol(next);
...
dd_MatrixPtr dd_PolyFile2Matrix (FILE *f, dd_ErrorType *Error)
{
 ...
 #if defined(HAVE_MSVC_SECURE_STRING_FNS)
  fscanf_s(f, "%ld %ld %s", &m_input, &d_input, numbtype);
 #else
...
*/
 ...
 /* This is to replace the line above, but it may not give 
  ...
  ptime_sec=ptime%60;
 #if defined(HAVE_MSVC_SECURE_STRING_FNS)
  localtime_s (&tmpTimeStruct, &starttime);
  asctime_s (sTmpBuffer, 32, &tmpTimeStruct);
  fprintf(f,"* Computation started at %s", sTmpBuffer);
  localtime_s (&tmpTimeStruct, &endtime);
  asctime_s (sTmpBuffer, 32, &tmpTimeStruct);
  fprintf(f,"*             ended   at %s", sTmpBuffer);
 #else
